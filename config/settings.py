...
# MediaCrawler 每个关键词爬取的数量
MEDIA_CRAWLER_MAX_NOTES_COUNT = 20  # 设置为20的倍数（20、40、60...）
...
# 启用的数据源列表，可选值：
# - "wechat": 微信公众号（搜狗微信搜索）
# - "xiaohongshu": 小红书（使用 MediaCrawler，需配置浏览器）
# - "zhihu": 知乎（使用 MediaCrawler，需配置浏览器）
# - "general": 通用资讯（可扩展）
# - "demo": 演示采集器（使用示例数据，测试用）
ENABLED_SOURCES = [
    # "demo",        # 演示采集器（使用示例数据）
    "wechat",      # 公众号（搜狗微信搜索）
    "xiaohongshu", # 小红书（使用 MediaCrawler）
    # "zhihu",       # 知乎（使用 MediaCrawler）
    # "general",     # 通用资讯（可扩展）
]
