...
**Tech Stack:** Python 3.10+, Pydantic, PyYAML, python-dotenv, Playwright, MediaCrawler, pytest.
...
- Modify: `requirements.txt`
  Include test dependency and align versions so root + MediaCrawler installation is predictable.
- Modify: `README.md`
...
def check_node() -> bool:
    ...
    if not node:
        print("WARN: Node.js not found. Zhihu/MediaCrawler may fail; install Node.js >= 16.")
        return False
...
- [ ] **Step 2: Make root requirements compatible with MediaCrawler**

Use versions that work after installing `MediaCrawler/requirements.txt`:
