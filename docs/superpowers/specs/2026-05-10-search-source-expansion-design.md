...
- 为开源整理 `MediaCrawler` 依赖，避免把 189M 的第三方源码直接内嵌在主仓库里。
...
`MediaCrawler` 已支持 `--type creator`，并且命令行参数 `--creator_id` 会在小红书平台写入 `config.XHS_CREATOR_ID_LIST`。当前 `XiaohongshuCrawler` 只调用 `--type search`，需要扩展为：
...
`MediaCrawler` 代码中有 `ZhihuCrawler.get_creators_and_notes()` 和 `config.ZHIHU_CREATOR_URL_LIST`，但命令行 `--creator_id` 当前没有把 `zhihu` 映射到 `ZHIHU_CREATOR_URL_LIST`。实现有两个选择：

- 方案 A：修改 `MediaCrawler/cmd_arg/arg.py`，让 `--creator_id` 在 `platform == zhihu` 时写入 `config.ZHIHU_CREATOR_URL_LIST`。
- 方案 B：在外层 wrapper 临时改写 `MediaCrawler/config/zhihu_config.py`。

推荐方案 A。它是很小的补丁，职责清楚，并且以后所有 `MediaCrawler` 命令行调用都一致。方案 B 会写第三方配置文件，容易污染工作区。
...
当前 `MediaCrawler` 是被 Git 直接跟踪的普通目录，不是 submodule，目录大小约 189M。它的许可证是 `NON-COMMERCIAL LEARNING LICENSE 1.1`，限制非商业学习和研究用途。开源仓库如果继续内嵌，会带来三个问题：

- 仓库体积大。
- 主仓库看起来像重新分发第三方爬虫。
- 许可证边界会让项目定位不清楚。

推荐把 `MediaCrawler` 外置为“运行时依赖目录”，而不是复制进主仓库：
...
AITrend/
  ...
  third_party/          # gitignored
    MediaCrawler/       # bootstrap clone or user-provided path
```
...
- 如果 `media_crawler.dir` 已存在，安装它的 requirements。
- 如果不存在，提示用户 clone MediaCrawler，或在用户同意执行时 clone 到 `third_party/MediaCrawler`。
- 报告登录态位置从 `MediaCrawler/browser_data` 改为 `third_party/MediaCrawler/browser_data`。

如果要保留同一个 Git 历史，只删除当前目录不会让历史里的 189M 消失。真正开源时建议新建干净仓库，或用历史重写工具清掉 vendor 历史。这个动作应独立于搜索功能实现，避免把业务改动和仓库历史治理混在一起。
...
- `MediaCrawler/cmd_arg/arg.py`：补齐 `zhihu` 的 `--creator_id` 映射。
- `requirements.txt`：新增 `gnews`。
- `scripts/bootstrap.py`：适配外置 `MediaCrawler` 路径。
- `config.yaml.example`、`.env.example`、`README.md`：更新迁移和登录说明。
- `tests/`：补配置校验、settings 导出、Google News 转换、小红书/知乎命令构造测试。

预计删除或迁移：

- 第一阶段不删除 `MediaCrawler/`，先让新配置支持外置路径。
- 第二阶段开源前删除 vendor 目录或创建干净仓库。
...
- 知乎配置 `creator_urls` 时，wrapper 会构造 `--type creator --creator_id ...`，并且 `MediaCrawler` 命令行能把 creator 列表映射到 `ZHIHU_CREATOR_URL_LIST`。
- `python main.py search --sources wechat,google_news` 能从配置读取关键词。
- README 明确账号采集必须使用主页 URL/ID，不承诺昵称自动解析。

## 风险

- Google News 在国内网络环境依赖代理，开源 README 要写清楚 `GOOGLE_NEWS_PROXY_URL`。
- 小红书账号 URL 缺少 `xsec_token` 时不属于第一版支持口径，文档必须要求从网页登录后的账号主页复制完整 URL。
- 修改 `MediaCrawler/cmd_arg/arg.py` 属于第三方源码补丁。外置之后如果直接 clone upstream，需要保存一个小 patch 或 fork。
- 删除内嵌 `MediaCrawler` 会影响当前登录态路径，必须在 README 说明登录态不可迁移或需要重新登录。
