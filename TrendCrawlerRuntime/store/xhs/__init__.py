...
# Repository: https://internal.local/TrendCrawlerRuntime/blob/main/store/xhs/__init__.py
...
async def update_xhs_note(note_item: Dict):
    ...
    """
    # 时间过滤: only save notes within the specified time range
    if config.ENABLE_TIME_FILTER and hasattr(config, 'TIME_FILTER_HOURS'):
        note_time = note_item.get("time")
        if note_time:
            import time as time_module
            from datetime import datetime, timedelta
            
            # Convert note time to timestamp (handle both seconds and milliseconds)
            if note_time > 1000000000000:  # Millisecond timestamp
                note_timestamp = note_time / 1000
            else:  # Second timestamp
                note_timestamp = note_time
            
            # Calculate cutoff time
            current_time = time_module.time()
            cutoff_time = current_time - (config.TIME_FILTER_HOURS * 3600)
            
            # Skip notes outside the time range
            if note_timestamp < cutoff_time:
                utils.logger.info(f"[store.xhs.update_xhs_note] Skip note {note_item.get('note_id')} due to time filter (published at {note_timestamp})")
                return
    ...
    local_db_item = {
        ...
        "last_modify_ts": utils.get_current_timestamp(),  # Last modification timestamp (Generated by TrendCrawlerRuntime, mainly used to record the latest update time of a record in DB storage)
