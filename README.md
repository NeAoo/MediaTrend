...
│   ├── manager.py               # 爬虫管理器
│   ├── wechat.py                # 微信公众号爬虫
│   ├── zhihu.py                 # 知乎爬虫（调用 TrendCrawlerRuntime）
│   └── xiaohongshu.py           # 小红书爬虫（调用 TrendCrawlerRuntime）
├── merger/                      # 数据合并模块
│   ├── __init__.py
...
│   ├── scheduler.py             # 定时调度器
│   └── monitor.py               # 监控模块
├── TrendCrawlerRuntime/                # 运行时目录框架（小红书、知乎等）
│   ├── config/
│   │   └── base_config.py       # TrendCrawlerRuntime 配置
│   └── ...
...
如果你要启用 `xiaohongshu` 或 `zhihu`，还需要额外安装 `TrendCrawlerRuntime` 依赖：

```bash
pip install -r TrendCrawlerRuntime/requirements.txt
```
...
# TrendCrawlerRuntime Cookie 配置（可选，目前使用扫码登录）
...
如果你要跑 `TrendCrawlerRuntime` 的 `xiaohongshu` / `zhihu`，它内部会使用自己的浏览器控制逻辑，第一次登录通常需要扫码。
