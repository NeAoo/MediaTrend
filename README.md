...
开源仓库不建议直接内嵌 `MediaCrawler`。默认配置使用 `./third_party/MediaCrawler`，可以手动 clone `MediaCrawler` 到该目录，也可以把 `media_crawler.dir` 指向你本机已有的 `MediaCrawler` checkout。`MediaCrawler` 使用非商业学习用途许可，使用前请阅读它的 `LICENSE`。
...
- 创建运行目录。
- 安装根项目依赖。
- 安装 `media_crawler.dir` 指向的 `MediaCrawler` 依赖。
...
```text
browser_data/
third_party/MediaCrawler/browser_data/
third_party/MediaCrawler/data/
raw_data/
merged_data/
scored_data/
output/
