...
1️⃣  数据采集 (CrawlerManager)
    ├─ 小红书 (xiaohongshu) - TrendCrawlerRuntime框架 
    ├─ 知乎 (zhihu) - TrendCrawlerRuntime框架
    ├─ 微信公众号 (wechat) - 搜狗微信搜索
    └─ 通用资讯 (general) - 可扩展
...
小红书平台通过 TrendCrawlerRuntime 采集的原始 JSONL 数据包含以下字段：
...
知乎平台通过 TrendCrawlerRuntime 采集的原始 JSONL 数据包含以下字段：
